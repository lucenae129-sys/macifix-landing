...
function AnimatedNumber({ value, suffix, color }) {
  ...
  return (
    <div ref={ref} className={`text-5xl md:text-6xl font-bold tracking-tighter ${color}`}>
      <motion.span>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 md:gap-8 text-center">
            {stats.map((stat, i) => (
              ...
              >
                ...
                <div>
                  <p className="text-white font-bold text-base tracking-tight">{stat.label}</p>
                  <p className="text-[#86868b] text-sm font-light mt-1">{stat.sublabel}</p>
                </div>
              </motion.div>
            ))}
          </div>
